...
function Fala23({ route, navigation }) {
  const { text } = route.params;
    ...
    <Text style={{fontSize: 20, color: "black", top:240, position: 'absolute', left: 80}}>Nina</Text>
        <Text style={{fontSize: 16, color: "black", top: 265, position: 'absolute', left: "12%", fontFamily: 'LangarRegular'}}>    Bom, mas me fala, em que eu posso te ajudar hoje?
       
</Text>     
    </View>              
   
   <TouchableOpacity style={{top: 280, position: 'absolute', left: 720}} onPress={() => {navigation.navigate('Fala24'   , { text: text})}}>
   ...
   <TouchableOpacity style={{top: 275, position: 'absolute', left:-25}} onPress={() => {
          navigation.navigate('Fala22'   , { text: text})}}>
    <Image style={{  height:40, width: 70, resizeMode:"contain"}} 
